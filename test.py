...
class Main(object):
	...
	def __init__(self, params):
		...
		self.positive_drug = self.load_data()
	...
	def load_data(self):
		...
		for split in ['train', 'test', 'valid', tt_file]:
			for line in open('/root/GPKG/data/{}/{}.tsv'.format(self.p.dataset, split)):
				sub, rel, obj = map(str.lower, line.strip().split('\t'))
				for line1 in open('/root/GPKG/data/test_data/p_drug.tsv'):
					sub1, rel1, obj1 = map(str.lower, line1.strip().split('\t'))
					if sub==sub1 and rel==rel1 and obj==obj1:
						sub1, rel1, obj1 = self.ent2id[sub], self.rel2id[rel], self.ent2id[obj]
						positive_drug.append((sub1, rel1, obj1))
	...
	def testpredict(self, split=''):
		...
		#save_path = os.path.join('/root/GPKG/code/model_saved', 'testrun_5605d68c')
		save_path = os.path.join('/root/GPKG/code/model_saved', 'testrun_900fa0c9')
		...
		with torch.no_grad():
			...
			#f=open(self.p.save_result,'a')
			for sr in t:
					if sr in self.positive_drug:
						label_np = np.append(label_np, 1)
					else: label_np = np.append(label_np, 0)
			lr_precision, lr_recall, _ = precision_recall_curve(label_np, pred_np)    
#   plt.plot([0,1], [no_skill, no_skill], linestyle='--')
			plt.plot(lr_recall, lr_precision, lw = 2, label= 'KG-Predict' + ' (AUPR = %0.3f)' % average_precision_score(label_np, pred_np))
			fontsize = 14
			plt.xlabel('Recall', fontsize = fontsize)
			plt.ylabel('Precision', fontsize = fontsize)
			plt.title('Precision Recall Curve')
			plt.legend()
			plt.savefig('KG-Predict_AUPR'+'.png')
			# list_a = label_np.tolist()
			# list_b = pred_np.tolist()
	...
	# 		tpr = dict() 
	# 		roc_auc = dict()
	# 		fpr[0], tpr[0], _ = roc_curve(label_np, pred_np)
	# 		roc_auc[0] = auc(fpr[0], tpr[0])
	# 		lw = 2
	# 		plt.plot(fpr[0], tpr[0],
    #      		lw=lw, label= 'KG-Predict' + ' (area = %0.2f)' % roc_auc[0])
	...
	# 		plt.savefig('Figure_KG_Predict' + ".png")
...
if __name__ == "__main__":
	...
	parser.add_argument('-data',           dest='dataset',         default='test_data',            		help='Dataset to use for the experiment')
	...
	parser.add_argument('-test_file',      default='ad_pre_processed',    	     	help='test file name')
